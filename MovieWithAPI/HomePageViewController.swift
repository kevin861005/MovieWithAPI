    ...
    var imageInfo: ImageInfo? {
        didSet {
            DispatchQueue.main.async {
                self.movieListButton.isEnabled = true
            }
        }
    }
...
//    }
    ...
    override func viewDidLoad() {
        ...
        // 按鈕預設先不能編輯
        movieListButton.isEnabled = false
...
//  }
    ...
    @IBSegueAction func toMovieListSegue(_ coder: NSCoder) -> MovieListTableViewController? {
        ...
        movieListTableViewController?.imageInfo = self.imageInfo
    ...
    // 取得相片網域
    func fetchImage() {
        let urlString = "https://api.themoviedb.org/3/configuration?api_key=a23559d13fed39749e5b94e6fefd836e";
        ...
        if let url = URL(string: urlString) {
            URLSession.shared.dataTask(with: url) { data, response , error in
                if let data {
                    let decoder = JSONDecoder()
                    do {
                        let configurationResponse = try decoder.decode(ConfigurationResponse.self,
                                                                       from: data)

                        self.imageInfo = configurationResponse.images
